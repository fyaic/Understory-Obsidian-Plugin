...
可审查源码位于 `src/`，安装资产位于仓库根目录。
内置本地引擎快照由 `engine-provenance.json` 锁定。`1.13.0` 之后的每个
release 都必须记录 `fyaic/Understory-graphify-engine` 的准确上游 commit。
...
`npm run verify` 会运行 Obsidian 官方 lint、99 项自动化测试、确定性 bundle、发布元数据检查、bundle 语法检查和本地引擎 smoke test。
...
Git tag 必须与 manifest 版本完全一致。当前版本：`1.13.0`。
